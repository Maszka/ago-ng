      ...
      img.onload = function() {
         ...
         if (this.width > FileSnippet.IMAGE_SIZE.width && this.height > FileSnippet.IMAGE_SIZE.height) {
           self.imageChangedEvent = event;
            ...
            self.toastr.error(`Minimalna szerokość: ${FileSnippet.IMAGE_SIZE.width} px, minimalna wysokość: ${FileSnippet.IMAGE_SIZE.height} px.`, 'Błąd!');
         }
      }

      img.src = URL.createObjectURL(file);
    } else {
      this.toastr.error('Niedozwolony format pliku. Załaduj plik .jpg lub .png', 'Błąd!');
    }
  }
      ...
      reader.addEventListener('load', (event: any) => {
        ...
        this.imageService.uploadImage(this.selectedFile.file).subscribe(
          ...
          (errorResponse: HttpErrorResponse) => {
            this.toastr.error(errorResponse.error.errors[0].detail, 'Błąd!');
            this.onFailure();
          })
      });
